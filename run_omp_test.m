...
%% Reconstruct final two-target image from OMP result

alphaHat = ompResult.alphaHat(:);

if numel(alphaHat) ~= 2
    error('Expected exactly 2 recovered coefficients in ompResult.alphaHat.');
end
...
for k = 1:numCarriers
    ...
    % Reconstruct IF using the final OMP coefficients
    reconIF = ...
        alphaHat(1) * templateIF1{k} + ...
        alphaHat(2) * templateIF2{k};
...
%% Multi-carrier coherent fusion of reconstructed two-target image

[finalTwoPower, ...
 finalTwoComplex, ...
 finalTwoNormalizedStack] = ...
...
%% Plot final dual-target image

truthXY = vertcat(sceneTwo.targets.positionM);
truthXY = truthXY(:,1:2);

estimatedXY = ompResult.selectedPositionsM(:,1:2);

figure('Name', 'Final dual-target image', ...
       'Color', 'w', ...
       'Position', [220 120 760 560]);

plotFinalDualTargetImage( ...
    finalTwoImage, ...
    truthXY, ...
    estimatedXY, ...
    'Final multi-carrier dual-target image');

function plotFinalDualTargetImage(imageStruct, truthXY, estimatedXY, plotTitle)

P = double(imageStruct.coherentPower);
P = P ./ max(P(:), eps);

x = imageStruct.xGridM(:).';
y = imageStruct.yGridM(:);

imagesc(x, y, 10*log10(P + eps));
axis xy equal tight;
clim([-50 0]);
colormap turbo;

cb = colorbar;
cb.Label.String = 'Normalized power / dB';
